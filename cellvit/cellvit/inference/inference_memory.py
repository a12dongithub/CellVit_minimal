...
class CellViTInferenceMemory(CellViTInference):
    ...
    def process_wsi(
        ...
        **kwargs,
            ...
            for batch_num, batch in enumerate(wsi_inference_dataloader):
                ...
                if self.mixed_precision:
                    with torch.autocast(device_type="cuda", dtype=torch.float16):
                        predictions = self.model.forward(patches, retrieve_tokens=self.graph)
                else:
                    predictions = self.model.forward(patches, retrieve_tokens=self.graph)
        ...
        # Also filter generic positions if they exist and are not already filtered above
        # The original code structure suggests positions are in graph_data regardless of self.graph
        if not self.graph:
             graph_data["positions"] = [graph_data["positions"][idx_c] for idx_c in keep_idx]
